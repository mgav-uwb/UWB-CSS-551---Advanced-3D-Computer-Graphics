<!--
  ...
  name its origin in passing. SEQUENCING: the primary axis is HISTORIC (the
  eras, in order); the secondary axis within/across an era is TOPIC PROXIMITY —
  related concepts cluster (e.g. Era 4 groups framebuffer + double buffering +
  anti-aliasing + motion blur as "the raster machine and its sampling costs").
  FOCUS is the timeline and the concepts, NOT the scene. A small demo Cornell
  box (teapot + bunny) appears only as a TOOL inside concept demos (meshes,
  lighting, the pipeline); it is never itself a topic — there are no "meet our
  scene" or "our scene so far/after/complete" checkpoint slides, and the
  scene stills are not embedded. No math, no equations — scale facts (counts,
  times) are the only numbers. Every technical term is bolded exactly ONCE, at
  its introduction, then used plainly after.
  ...
  SCENE STILLS: figures/scene-stage0..4.png + scene-hero.png are GENERATED by
  tools/gen-scene-shots.mjs (the our-scene demo rendered headless). As of the
  concept-focus pass they are NO LONGER embedded anywhere in the deck — the
  scene is a tool, not a topic, and the live pipeline demo replaces every
  former checkpoint still. The generator is kept for regeneration if a still is
  ever wanted again; re-run it, never hand-edit the PNGs.
  ...
  Session plan (120 min, Tue 5:45-7:45 PM synchronous online):
    0:00  Open (title, Sketchpad, the goal, pixels+RGB, applications, timeline) ~9 min
    0:09  Era 1  Interactive pictures born (1950-68)                        6 min
    0:15  Era 2  The Utah school (1968-80) — a world in numbers, taking     30 min
                 the picture (pipeline demo), surfaces that lie
    0:45  Era 3  Chasing the photograph (1975-90) — GI, offline light       12 min
    0:57  Era 4  The raster machines (1980-2000) — framebuffer, double      15 min
                 buffering, silicon/GPU, aliasing/AA, motion blur
    1:12  Era 5  The programmable era (2000-15) — shaders, surfaces         14 min
...
<img src="figures/pixels-zoom.svg" alt="a coarse pixel grid on a screen, with a 3×3 patch magnified to show each pixel's red, green, blue numbers" style="max-height: 320px; width: auto;">

a **pixel** — *picture element* — is one colored square of the grid; each is three numbers, **RGB**
...
Every station at once — the whole assembly line runs live next. Drag `stage` through its first three stops.
...
The **framebuffer**: a block of memory holding every pixel's RGB — the monitor *scans it out* sixty times a second.

- a vector display drew lines; a **raster** display fills a grid — and needs somewhere to keep it
- "drawing" = write numbers into this array; the screen is just its viewer
- a laptop's is a few million numbers (~6 MB) — cheap now, exotic and costly in 1975

---

## Double buffering

Draw the next frame into a hidden **back buffer**; swap it on-screen only when it's finished. **Double buffering** kills the *tear*.

- draw straight into the live framebuffer and the monitor catches you mid-draw — a torn frame
- so keep two buffers: show one, draw the next, then flip
- flipping in sync with the monitor's refresh is **v-sync** — the root of every frame-rate setting
...
Anti-aliasing smooths sampling in *space*. **Motion blur** smooths it in *time*.

- one rendered frame is a single frozen instant — fast motion jumps between frames, and the eye sees strobing
- a real camera's shutter stays open for a slice of time, so anything moving *smears*
- so sample several instants per frame and average — the temporal twin of supersampling
