...
export function make(container, { stage = 'embed', controls } = {}) {
  const model = { bump: PARAMS.bump.value, lightAz: PARAMS.lightAz.value };
  ...
  wall.position.y = 1.5;
  wall.rotation.x = -0.12;    // slight tilt so the quad reads in perspective
  scene.add(wall);
  ...
  function update() {
    ...
    table.update('tris', 2);   // the whole punchline
    render();
  }
