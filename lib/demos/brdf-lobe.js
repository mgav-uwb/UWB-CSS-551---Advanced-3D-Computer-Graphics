...
const HELP_HTML = `
  ...
  <p>A Cornell box — white room with a red wall and a blue wall, an area light
  on the ceiling — holding the classic trio: a gold METAL sphere, a pastel
  PLASTIC cube, and a saturated glossy pyramid. It's lit like a radiosity
  render (the coloured walls bleed onto the objects) plus a headlight at the
  camera. All three share the roughness slider and Phong/GGX model, so the
  inset (click to swap) — the 2D reflectance "lobe" — describes the specular
  sharpness they all have in common.</p>
  <h4>The concept</h4>
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  // --- Cornell-box room: white floor/ceiling/back, a RED left wall and a BLUE
  // right wall, an area light on the ceiling — the classic radiosity setup.
  // Real-time three.js has no global illumination, so dim red/blue "bounce"
  // point lights near the coloured walls fake the colour bleed onto the
  // (near-white) objects.
  const HW = 4, RH = 6, HD = 4; // half-width, room height, half-depth (front open)
  const matte = (color) => new THREE.MeshStandardMaterial({ color, roughness: 1, metalness: 0, side: THREE.DoubleSide });
  const whiteMat = matte(0xd6d6d6);
  ...
  scene.add(plane(2 * HW, 2 * HD, whiteMat, [0, 0, 0], [-Math.PI / 2, 0, 0])); // floor
  scene.add(plane(2 * HW, 2 * HD, whiteMat, [0, RH, 0], [Math.PI / 2, 0, 0])); // ceiling
  scene.add(plane(2 * HW, RH, whiteMat, [0, RH / 2, -HD], [0, 0, 0])); // back
  scene.add(plane(2 * HD, RH, matte(0xcc3333), [-HW, RH / 2, 0], [0, Math.PI / 2, 0])); // left: red
  scene.add(plane(2 * HD, RH, matte(0x3355cc), [HW, RH / 2, 0], [0, -Math.PI / 2, 0])); // right: blue
  ...
  const panel = new THREE.Mesh(new THREE.PlaneGeometry(2.6, 2.6), new THREE.MeshBasicMaterial({ color: 0xffffff }));
  panel.position.set(0, RH - 0.02, 0);
  panel.rotation.x = Math.PI / 2;
  scene.add(panel);

  scene.add(new THREE.AmbientLight(0xffffff, 0.12));
  const ceiling = new THREE.PointLight(0xfff6e6, 55, 30);
  ceiling.position.set(0, RH - 0.4, 0);
  scene.add(ceiling);
  const redBounce = new THREE.PointLight(0xcc3333, 9, 14);
  redBounce.position.set(-HW + 0.4, RH / 2, 0.6);
  scene.add(redBounce);
  const blueBounce = new THREE.PointLight(0x3355cc, 9, 14);
  blueBounce.position.set(HW - 0.4, RH / 2, 0.6);
  scene.add(blueBounce);
  const headlight = new THREE.PointLight(0xffffff, 14, 40);
  scene.add(headlight); // parked at the camera every render (render3d)

  // The classic trio — sphere · cube · pyramid — each with its OWN material
  // identity (colour + metalness), so the scene shows three distinct BRDFs:
  //   • sphere  — gold METAL (metalness 1): reflects the room in its own colour
  //   • cube    — pastel PLASTIC (dielectric): soft body + a white highlight
  //   • pyramid — a saturated glossy dielectric ("something else")
  ...
  // per-object colour/metalness differ.
  const sphere = new THREE.Mesh(new THREE.SphereGeometry(1.0, 48, 32));
  sphere.position.set(2.1, 1.0, -0.6);
  const cube = new THREE.Mesh(new THREE.BoxGeometry(1.5, 1.5, 1.5));
  cube.position.set(-2.1, 0.75, -0.3);
  cube.rotation.y = 0.35;
  // A square pyramid: a 4-sided cone sitting flat on its base (apex up).
  const pyramid = new THREE.Mesh(new THREE.ConeGeometry(1.15, 2.0, 4));
  pyramid.position.set(0, 1.0, -1.0);
  pyramid.rotation.y = Math.PI / 4;
  const objects = [
    { mesh: sphere, color: 0xc9a24b, metalness: 1.0 }, // gold metal
    { mesh: cube, color: 0xf0b6c8, metalness: 0.0 }, // pastel-pink plastic
    { mesh: pyramid, color: 0x7a4dc7, metalness: 0.0 }, // saturated purple glossy
  ];
  ...
  function captureEnv() {
    ...
    // env map is the walls + ceiling light + colour bounce.
  ...
  // Observer framing: in front of the open box, looking in — so the red-left/
  // blue-right walls and the lit objects read like the reference render.
  const cam = makeOrbitCamera({
    camera,
    render: render3d,
    home: { eye: [0, 3.1, 9], target: [0, 2, -1] },
