...
//
// This demo mounts into the shared demo-shell chrome ("?"/legend, the demo's
// own controls in rail cards) like every other cockpit demo, but it is a 2D
// demo: there is no three.js scene, no orbit camera, and no "⚙" (a polar
// plot has no nav speeds and no grid/axes concept to toggle) — `nav: null`
// and `settings: []` reflect that. The demo's own <canvas> is mounted
// directly into shell.sceneEl, centered, in place of the old fixed-size
// demo-viewport div.
...
const HELP_HTML = `
  ...
  <p>The reflectance "lobe" of a BRDF: a 2D polar plot of how much light
  reflects toward each outgoing direction, for a fixed incoming light
  direction.</p>
  <h4>The concept</h4>
  <p>A BRDF describes how a surface scatters light. A shiny surface has a
  tight lobe around the mirror direction; a rough surface spreads light into
  a wide lobe. This shows Phong's cosine-power lobe versus a GGX-flavored
  microfacet lobe.</p>
  <h4>Try this</h4>
  <p>Lower the roughness (or raise the shininess) and watch the lobe tighten
  toward a mirror; switch between Phong and GGX to compare the tails.</p>
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  // No three.js scene, no orbit camera, no "⚙" (a 2D polar plot has no nav
  // speeds and no grid/axes concept) — nav:null and settings:[] suppress
  // the camera-gate machinery and the "⚙" button entirely.
  const shell = makeShell(container, {
    stage,
    help: { html: HELP_HTML },
    settings: [],
    legend: 'polar plot — radius = relative reflected radiance',
    nav: null,
  });

  // The demo's own 2D canvas mounts directly into the shell's scene pane,
  // centered (the scene pane is much larger than the plot's fixed 400×300).
  shell.sceneEl.style.display = 'flex';
  shell.sceneEl.style.alignItems = 'center';
  shell.sceneEl.style.justifyContent = 'center';
  ...
  // Rail cards: the controller (sliders + model toggle) and the readout.
