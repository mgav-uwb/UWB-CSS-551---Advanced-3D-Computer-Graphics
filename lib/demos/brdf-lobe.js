...
const BASE_COLOR = 0xe8e8e8; // near-white objects (radiosity-render look), so the
                             // coloured-wall bleed and the BRDF highlight read clearly

const HELP_HTML = `
  <h4>What you're seeing</h4>
  <p>A Cornell box — white room with a red wall and a blue wall, an area light
  on the ceiling — holding the classic trio (sphere, cube, pyramid), all
  wearing the SAME material. It's lit like a radiosity render (the coloured
  walls bleed onto the near-white objects) plus a headlight at the camera. The
  inset (click to swap) is the 2D reflectance "lobe" of that material's BRDF.</p>
  <h4>The concept</h4>
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  // The classic trio — sphere · cube · pyramid — in a row on the floor,
  // sharing ONE swappable BRDF material.
  ...
  const objects = [sphere, cube, pyramid];
