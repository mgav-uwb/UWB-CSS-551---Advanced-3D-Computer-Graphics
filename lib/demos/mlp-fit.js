...
// IN THE BROWSER by gradient descent on twelve (x, y) points. Training is
// LIVE and incremental: dragging `epochs` up continues training the current
// weights a few steps per animation frame, so the curve bends in front of
// the class, the epoch counter ticks, and a loss plot grows in the corner;
// dragging it down resets to the seeded random start and retrains. Nothing is
// precomputed. Between the points the function is smooth (it interpolates),
// which is the property the S10 ladder leans on: a learned denoiser is
// smooth where a lookup table is not.
...
import { TARGETS, makeSamples, makeMlp, trainStep, predict, mse } from '../core/mlp-fit.js';

const PARAMS = {
  epochs: { label: 'epochs', min: 0, max: 3000, step: 50, value: 0, format: (v) => v.toFixed(0) },
...
const HELP_HTML = `
  ...
  through them, live. Drag <code>epochs</code> up and the training runs in
  front of you, a few gradient steps per frame: the curve bends, the epoch
  counter ticks, and the loss plot in the corner grows. At 0 the network is a
  random wiggle; by a few hundred epochs it fits. Drag down to reset and
  retrain.</p>
  <h4>The concept</h4>
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  const disp = { truth: true, plot: true };
  let samples = makeSamples(model.target);
  let net = makeMlp(model.hidden);
  let epoch = 0;             // gradient steps applied to `net` so far
  let history = [];          // loss after each epoch, for the live plot
  let loss = mse(net, samples);
  let animHandle = null;
  let stepsPerFrame = 6;     // training speed (⚙): ~400 epochs in about a second

  const shell = makeShell(container, {
    stage, help: { html: HELP_HTML },
    settings: [
      { label: 'truth', toggle: true, title: 'Show the hidden curve the points were sampled from', getCurrent: () => disp.truth, apply: (v) => { disp.truth = v; draw(); } },
      { label: 'loss plot', toggle: true, title: 'Show the live loss-vs-epoch plot', getCurrent: () => disp.plot, apply: (v) => { disp.plot = v; draw(); } },
      { label: 'steps/frame', decimals: 0, title: 'Gradient steps per animation frame (training speed) — default 6', getCurrent: () => stepsPerFrame, apply: (v) => { stepsPerFrame = Math.max(1, Math.round(v)); } },
    ],
    legend: 'drag epochs — watch the network train, live', nav: null,
  });
  ...
  const table = new ValueTable(readoutCard, {
    rows: [
      ...
      { id: 'epochs', label: 'epochs run', format: (v) => v },
  ...
  note.textContent = 'f(x) = Σ w2·tanh(w1·x + b1) + b2; full-batch gradient descent runs live, a few steps per frame, from a seeded random start';
  readoutCard.appendChild(note);

  function reset() {
    samples = makeSamples(model.target);
    net = makeMlp(model.hidden);
    epoch = 0; history = []; loss = mse(net, samples);
  }
  // Continue training toward model.epochs, a few steps per frame, redrawing
  // as it goes. If the target dropped below the epochs already run, or the
  // architecture/data changed, restart from the seeded initialization.
  function recompute(restart = false) {
    if (animHandle) { cancelAnimationFrame(animHandle); animHandle = null; }
    if (restart || model.epochs < epoch) reset();
    draw();
    const tick = () => {
      const n = Math.min(stepsPerFrame, model.epochs - epoch);
      for (let i = 0; i < n; i++) { trainStep(net, samples); epoch++; history.push(mse(net, samples)); }
      loss = mse(net, samples);
      draw();
      animHandle = epoch < model.epochs ? requestAnimationFrame(tick) : null;
    };
    if (epoch < model.epochs) animHandle = requestAnimationFrame(tick);
  }
  function draw() {
    ...
    for (let i = 0; i <= 300; i++) { const x = XMIN + ((XMAX - XMIN) * i) / 300; const y = Math.max(YMIN, Math.min(YMAX, predict(net, x))); i ? ctx.lineTo(toX(x), toY(y)) : ctx.moveTo(toX(x), toY(y)); }
    ctx.stroke();
    const { xs, ys } = samples;
    ...
    // live loss plot (log scale), bottom-right inset
    if (disp.plot) {
      const iw = Math.round(pw * 0.34), ih = Math.round(ph * 0.3), ix = px + pw - iw - 10, iy = py + ph - ih - 10;
      ctx.fillStyle = 'rgba(23,27,38,0.85)'; ctx.fillRect(ix, iy, iw, ih);
      ctx.strokeStyle = 'rgba(140,150,175,0.5)'; ctx.lineWidth = 1; ctx.strokeRect(ix, iy, iw, ih);
      ctx.fillStyle = '#8b93a7'; ctx.font = '12px system-ui, sans-serif';
      ctx.fillText(`loss vs epoch (log)  ${epoch < model.epochs ? 'training…' : epoch ? 'done' : ''}`, ix + 6, iy + 14);
      if (history.length > 1) {
        const lo = Math.log10(Math.max(1e-6, Math.min(...history))), hi = Math.log10(Math.max(...history) || 1);
        const span = Math.max(model.epochs, history.length, 1);
        ctx.strokeStyle = '#ffb400'; ctx.lineWidth = 2; ctx.beginPath();
        history.forEach((l, i) => { const x = ix + 6 + ((iw - 12) * i) / span; const y = iy + ih - 6 - ((ih - 24) * (Math.log10(Math.max(1e-6, l)) - lo)) / (hi - lo || 1); i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); });
        ctx.stroke();
      }
    }
    table.update('units', model.hidden); table.update('weights', 3 * model.hidden + 1); table.update('epochs', `${epoch} / ${model.epochs}`); table.update('loss', loss);
    table.update('state', epoch < model.epochs ? 'training…' : epoch === 0 ? 'untrained (random weights)' : 'trained');
  }
