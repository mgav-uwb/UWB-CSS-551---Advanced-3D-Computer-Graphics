...
// from blocky to smooth) and `angle` (rotate the triangle — the stair-step
// aliasing crawls along the edges). ⚙ toggles: pixel grid lines, the sampled
...
export function make(container, { stage = 'embed', controls } = {}) {
  const model = { res: PARAMS.res.value, angle: PARAMS.angle.value };
  ...
  function draw() {
    ...
    for (let r = 0; r < n; r++) {
      for (let c = 0; c < n; c++) {
        const p = [(c + 0.5) / n, 1 - (r + 0.5) / n]; // pixel center, unit space
        ...
        if (inside) {
          covered += 1;
          const R = Math.round(w0 * TRI_COLORS[0][0] + w1 * TRI_COLORS[1][0] + w2 * TRI_COLORS[2][0]);
          const G = Math.round(w0 * TRI_COLORS[0][1] + w1 * TRI_COLORS[1][1] + w2 * TRI_COLORS[2][1]);
          const B = Math.round(w0 * TRI_COLORS[0][2] + w1 * TRI_COLORS[1][2] + w2 * TRI_COLORS[2][2]);
