 ...
 * force a resync after changing `fly` out from under this field. Optional
 * `title` sets a native tooltip on the row (used by the Settings panel's
 * abbreviated labels — e.g. "sens" — to spell out units without widening
 * the fixed-width label column).
 */
function bindNumberField(container, { label, decimals = 0, getCurrent, apply, title }) {
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  // --- Help ("?") button + panel (Fix 3): present in BOTH stages, always
  // built (not gated behind stage:'full' like the View/Camera-boxes/
  // Viewpoints cockpit cards) — this is orientation content for a viewer,
  // not a camera control. Deliberately NOT namespaced under .cockpit (same
  // reasoning as .gsplat-explore/.gsplat-exit above it in lib.css): the
  // bare lib/_gsplat-embed-test/ fixture mounts .gsplat-scene without a
  // .cockpit-classed ancestor, and the help button must still render there.
  const helpBtn = document.createElement('button');
  helpBtn.type = 'button';
  helpBtn.className = 'gsplat-help-btn';
  helpBtn.textContent = '?';
  helpBtn.setAttribute('aria-label', 'About this demo');
  viewportEl.appendChild(helpBtn);

  const helpPanel = document.createElement('div');
  helpPanel.className = 'gsplat-help-panel';
  helpPanel.style.display = 'none';
  helpPanel.setAttribute('role', 'dialog');
  helpPanel.setAttribute('aria-label', 'About this demo');
  helpPanel.innerHTML = `
    <button type="button" class="gsplat-help-panel-close" aria-label="Close">×</button>
    ...
    triangle.</p>
    <p class="gsplat-help-panel-hint">Esc, ×, or click outside to close.</p>
  `;
  ...
  function setHelpOpen(open) {
    ...
    // Mutual exclusion with the Settings panel (built below) — safe to call
    // here even though setSettingsOpen is defined later in this function:
    // both are `function` declarations, hoisted across the whole of make(),
    // and this call only ever runs from a later click, never during this
    // synchronous setup.
    if (open) setSettingsOpen(false);
  }
  setHelpOpen(false);

  // Both the button and the panel stop mousedown propagation so opening/
  // reading the panel never reaches viewportEl's own mousedown handler
  // (onSceneMouseDown, wired below in the InputController section) — that
  // handler starts click-drag mouse-look, which must not fire just because
  // the user clicked "?" or a line of help text.
  ...
  helpPanel.querySelector('.gsplat-help-panel-close').addEventListener('click', (e) => {
    e.stopPropagation();
    setHelpOpen(false);
  });
  ...
  // Esc-to-close, scoped: registered in the CAPTURE phase (fires before any
  // bubble-phase document keydown listener, including InputController's
  // onKeyDown below) and stops the event outright whenever the panel is
  // open — so an Esc that closes the help panel can NEVER also reach (and
  // be misread by) the nav Esc-handling in onKeyDown (Fix 4's embed-only
  // deactivate). When the panel is closed this listener is a no-op and Esc
  // falls through to nav exactly as before.
  ...
  // settings: user-tunable control-FEEL knobs (Settings-menu task). Distinct
  // from `fly` — `fly` is camera STATE (saved/restored wholesale by Home and
  // viewpoint snapshots); these are session-scoped control-sensitivity/step
  // knobs, never snapshotted, edited only via the Settings ("⚙") panel
  // below. mouseSensitivity's default is lowered from the old flat
  // MOUSE_SENS=0.25 deg/px (reported too fast) to 0.12. fly.moveSpeed
  // itself stays the single source of truth for WASD speed (this object
  // does NOT duplicate it) — the Settings panel edits fly.moveSpeed
  // directly, same as the Camera-card box it replaces.
  const settings = {
  ...
  // --- Settings ("⚙") button + panel: SAME closeable-panel idiom as the
  // help ("?") button above — absolute-positioned inside viewportEl (adds
  // NOTHING to slide scrollHeight), closeable via ×/Esc/click-outside, and
  // present in BOTH stages (built unconditionally, like help). Distinct
  // concern from help (orientation content): this exposes `settings` (plus
  // fly.moveSpeed, the single source of truth for WASD speed) as validated
  // NUMBER fields via the same bindNumberField used elsewhere — deliberately
  // NOT range sliders, so the FOV slider stays the only input[type=range]
  // in the host (see file header + tools/test-demos.mjs, tools/
  // test-gsplat-layout.mjs, both of which assert exactly one). Its fields
  // and the panel itself live inside viewportEl (i.e. inside `container`),
  // so the onSceneBlur relatedTarget guard below already treats focus
  // staying on any of them as "still in-container" — opening/closing/typing
  // in Settings can never tear nav down, same as the help panel.
  const settingsBtn = document.createElement('button');
  settingsBtn.type = 'button';
  settingsBtn.className = 'gsplat-settings-btn';
  settingsBtn.textContent = '⚙';
  settingsBtn.setAttribute('aria-label', 'Settings');
  viewportEl.appendChild(settingsBtn);

  const settingsPanel = document.createElement('div');
  settingsPanel.className = 'gsplat-settings-panel';
  settingsPanel.style.display = 'none';
  settingsPanel.setAttribute('role', 'dialog');
  settingsPanel.setAttribute('aria-label', 'Settings');
  settingsPanel.innerHTML = `
    <button type="button" class="gsplat-settings-panel-close" aria-label="Close">×</button>
    <h4>Settings</h4>
  `;
  const settingsFieldsEl = document.createElement('div');
  settingsFieldsEl.className = 'gsplat-settings-fields';
  settingsPanel.appendChild(settingsFieldsEl);
  const settingsHint = document.createElement('p');
  settingsHint.className = 'gsplat-settings-panel-hint';
  settingsHint.textContent = 'Esc, ×, or click outside to close.';
  settingsPanel.appendChild(settingsHint);
  viewportEl.appendChild(settingsPanel);

  // Each field: default/range per the brief. mouseSensitivity's default
  // (0.12) is the lowered-from-0.25 value seeded into `settings` above.
  bindNumberField(settingsFieldsEl, {
    label: 'sens', decimals: 2, title: 'Mouse-look sensitivity, deg/px (click-drag and pointer-lock alike) — default 0.12, range 0.02-0.5',
    getCurrent: () => settings.mouseSensitivity,
    apply: (v) => { settings.mouseSensitivity = clamp(v, 0.02, 0.5); },
  });
  bindNumberField(settingsFieldsEl, {
    label: 'speed', decimals: 1, title: 'WASD move speed, units/sec — default 3, range 0.1-20',
    getCurrent: () => fly.moveSpeed,
  ...
  bindNumberField(settingsFieldsEl, {
    ...
    getCurrent: () => settings.rollRate,
    apply: (v) => { settings.rollRate = clamp(v, 10, 180); },
  });
  bindNumberField(settingsFieldsEl, {
    label: 'zoom', decimals: 0, title: 'Zoom +/- step, deg FOV per click — default 5, range 1-20',
    getCurrent: () => settings.zoomStep,
    apply: (v) => { settings.zoomStep = clamp(v, 1, 20); },
  });
  bindNumberField(settingsFieldsEl, {
    label: 'turn', decimals: 0, title: 'Yaw/pitch/roll nudge-button step, deg per click — default 10, range 1-45',
    getCurrent: () => settings.turnStep,
    apply: (v) => { settings.turnStep = clamp(v, 1, 45); },
  });
  ...
  function setSettingsOpen(open) {
    ...
    if (open) setHelpOpen(false); // mutual exclusion — only one panel open at a time
  }
  setSettingsOpen(false);

  // Same mousedown-stopPropagation / click-outside / capture-phase-Esc
  // idiom as the help button+panel above, for the same reason: opening/
  // reading/editing Settings must never fire onSceneMouseDown's click-drag
  // mouse-look, and an Esc that closes Settings must not also reach (and be
  // misread by) onKeyDown's nav Esc-handling.
  ...
  settingsPanel.querySelector('.gsplat-settings-panel-close').addEventListener('click', (e) => {
    e.stopPropagation();
    setSettingsOpen(false);
  });
  document.addEventListener('mousedown', (e) => {
  ...
  // --- Controls legend: a compact, always-visible strip of the free-fly
  // key bindings along the TOP of the scene. SANDBOX (stage:'full') ONLY —
  // the embed teaser stays clean and slide-safe (matches the View/Camera-
  // boxes/Viewpoints cockpit cards' stage:'full' gating above). Absolute-
  // positioned inside viewportEl (adds nothing to layout height, same idiom
  // as the help/settings buttons+panels) and anchored top-LEFT — .gsplat-
  // help-btn/.gsplat-settings-btn already own the top-right corner (see
  // .gsplat-exit's right:64px comment in lib.css), so top-left is the only
  // corner that can never overlap them.
  if (stage === 'full') {
    const legendEl = document.createElement('div');
    legendEl.className = 'gsplat-legend';
    legendEl.textContent = 'WASD move · Q/E roll · Space/Ctrl up·down · drag to look · Capture mouse (Esc release)';
