...
const HELP_HTML = `
  ...
  clip planes and its field of view. You are ORBITING an OBSERVER camera to
  inspect a SECOND, pictured camera's frustum — two different cameras.</p>
  <h4>The concept</h4>
  <p>Perspective projection maps everything inside the frustum into the
  image; objects nearer the pictured camera project larger. Near/far bound
  what's visible; FOV sets how wide.</p>
  <h4>Try this</h4>
  <p>Narrow the FOV slider and watch the frustum pinch; change near/far and
  watch the clip planes slide. Drag to orbit the observer; scroll to zoom.</p>
...
export function make(container, { stage = 'embed', controls } = {}) {
  ...
  // fixed 400×300 box — that's what makes it scene-dominant. sc.camera is
  // the OBSERVER (third-person render camera) below; the SUBJECT camera
  // above (SUBJECT_EYE/SUBJECT_VIEW) is math-only and never rendered from.
